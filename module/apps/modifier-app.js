 ...
 */
export default class d100AModifierApplication extends FormApplication {
...
console.log("HERE--",html)
        ...
        html.find('.modifier-effect-type select').change(event => {
            ...
            switch (effectType) {
                ...
                //    for (const armorClass of Object.entries(CONFIG.SFRPG.modifierArmorClassAffectedValues)) {
                   ...
                   // d100Alternity addition 
                case SFRPGEffectType.HIT_POINTS:
                        target.prop('disabled', false);
                        target.find('option').remove();
                        for (const [k,v] of Object.entries(CONFIG.d100A.modifierHitPointsAffectedValues)) {
                            target.append(`<option value="${k}">${game.i18n.localize(v)}</option>`)
                        }
                        break;    

                        case SFRPGEffectType.INITIATIVE:
                        target.prop('disabled', false);
                        target.find('option').remove();
                        for (const [k,v] of Object.entries(CONFIG.d100A.modifierActionCheckAffectedValues)) {
                            target.append(`<option value="${k}">${game.i18n.localize(v)}</option>`)
                        }
                        break;    
    ...
    async _render(...args) {
        ...
        console.log(  "SFRPGEffectType",SFRPGEffectType,effectType,valueAffectedElement)
        switch (effectType) {
            case SFRPGEffectType.ABILITY_SKILLS:
