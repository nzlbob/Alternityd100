...
export function computeCompoundBulkForItem(item, contents) {
    let contentBulk = 0;
   // console.log(["computeCompoundBulk", item, contents]);
    if (item?.system?.container?.storage && item.system.container.storage.length > 0) {
        for (let storage of item.system.container.storage) {
            let storageIndex = item.system.container.storage.indexOf(storage);
            let storageBulk = 0;

            let storedItems = contents.filter(x => item.system.container.contents.find(y => y.id === x.item._id && y.index === storageIndex));
    ...
    let personalBulk = 0;
    if (item?.system?.bulk) {
        if (item.system.bulk.toUpperCase() === "L") {
            personalBulk = 1;
        } else if (!Number.isNaN(Number.parseInt(item.system.bulk))) {
            personalBulk = item.system.bulk * 10;
        }

        if (item.system.quantity && !Number.isNaN(Number.parseInt(item.system.quantity))) {
            // Compute number of packs based on quantityPerPack, provided quantityPerPack is set to a value.
            let packs = 1;
            if (item.system.quantityPerPack === null || item.system.quantityPerPack === undefined) {
                packs = item.system.quantity;
            } else {
                if (item.system.quantityPerPack <= 0) {
                    packs = 0;
                } else {
                    packs = Math.floor(item.system.quantity / item.system.quantityPerPack);
                }
            }
        ...
        if (item.system.equipped) {
            if (item.system.equippedBulkMultiplier !== undefined && !Number.isNaN(Number.parseInt(item.system.equippedBulkMultiplier))) {
                personalBulk *= item.system.equippedBulkMultiplier;
            }
        }
    }
    ...
    while (itemsToTest.length > 0) {
        ...
        if (draggedItem.data?.container?.contents?.length > 0) {
            ...
            for (const {id:contentItemId, index:contentItemIndex} of draggedItem.system.container.contents) {
                if (contentItemId) {
                    const contentItem = source.token.system.flags.sfrpg.itemCollection.items.find(x => x._id == contentItemId);
                    if (contentItem) {
