...
export class d100Combat extends foundry.documents.Combat {
  static HiddenTurn = 0;

  async _cleanupOrdnanceTokensOnCombatEnd() {
    try {
      if (!game.user?.isGM) return;

      const scene = this.scene ?? (this.sceneId ? game.scenes?.get?.(this.sceneId) : null) ?? canvas?.scene;
      if (!scene) return;
      ...
      for (const c of combatants) {
        ...
        const tokenFlagsOrdnance = tokenDoc?.flags?.d100A?.ordnance;

        const crewRoleRaw = c?.flags?.d100A?.crewRole;
        const crewRole = crewRoleRaw != null ? String(crewRoleRaw).trim() : "";

        const tokenActorType = tokenDoc?.actor?.type ?? tokenDoc?.actor?.system?.type ?? null;
        const combatantActorType = c?.actor?.type ?? c?.actor?.system?.type ?? null;
        const linkedActorType = (c?.actorId ? (game.actors?.get?.(c.actorId)?.type ?? game.actors?.get?.(c.actorId)?.system?.type) : null);

        const isOrdnance = Boolean(
          tokenFlagsOrdnance
          || crewRole === "ordnance"
          || tokenActorType === "ordnance"
          || combatantActorType === "ordnance"
          || linkedActorType === "ordnance"
        );

        if (isOrdnance) tokenIds.add(tokenId);
      }
      ...
      console.error("d100Combat: failed to cleanup ordnance tokens on combat end", err);
    }
  }

  /** @override */
  async _preDelete(options, userId) {
    await this._cleanupOrdnanceTokensOnCombatEnd();
    return super._preDelete(options, userId);
  }

  async _tickOrdnanceLifetimePiloting() {
    try {
      if (this.getCombatType?.() !== "starship") return;
      const thisSubPhase = this.getSubPhases?.()[this.action ?? 0];
      if (!thisSubPhase?.piloting) return;
      ...
      for (const t of tokens) {
        const ord = t?.flags?.d100A?.ordnance;
        const lifetime = ord?.lifetime;
        if (!lifetime || lifetime.type !== "duration") continue;

        const remaining = Number(lifetime.roundsRemaining);
        ...
        if (next <= 0) {
          ...
          updates.push({
            _id: t.id,
            "flags.d100A.ordnance.lifetime.roundsRemaining": next,
            // Mirror to the ordnance actor schema so existing sheet/buttons remain coherent.
            "delta.system.attributes.duration.value": next
          });
        }
      }
      ...
      console.error("d100Combat: failed to tick ordnance lifetime", err);
    }
  }
  /**
   * 
  ...
  _preCreate(data, options, user) {
   // console.log(this.scene)
      console.log("d100Combat - _preCreate", game.scenes.active.isStarship)
    const combatType = game.scenes.active.isStarship ? "starship" : "normal"
    const update = {
...
console.log("\nstartCombat() -->") 
    ...
    const updates = this.combatants.map(c => {
      const linkedActor = game.actors.get(c.actorId) || null;
      const isNpcCrew = c.isNpcCrew ? true : false;
      const flagCrewRoleRaw = c.flags?.d100A?.crewRole;
      const flagCrewRole = flagCrewRoleRaw != null ? String(flagCrewRoleRaw).trim() : "";
      const isOrdnance = linkedActor?.type === "ordnance" || linkedActor?.system?.type === "ordnance";
      const crewRole = (flagCrewRole ? flagCrewRole : null)
        ?? (isOrdnance ? "ordnance" : null)
        ?? c.token?.actor?.findCrewJob?.(c.actorId)
        ?? c.crewRole
        ?? null;

      const update = {
        _id: c.id,
        "flags.d100A.isNpcCrew": isNpcCrew,
        "flags.d100A.actions.total": c.apr,
        "flags.d100A.actions.remaining": c.apr,
        "flags.d100A.delayed": false,
        "flags.d100A.stunned.isStunned": false, // This should be a look at the  get isInitStunned()
        "flags.d100A.stunned.stunnedRound": -1,
        "flags.d100A.stunned.stunDur": 0
      };

      // Preserve or infer crew role for starship sub-phase gating.
      if (crewRole != null && String(crewRole).trim() !== "") update["flags.d100A.crewRole"] = crewRole;
  ...
  nextTurnStatus() {
    ...
    const whoCanActNormalized = (whoCanAct ?? []).map(r => String(r).toLowerCase());
    //   console.log(whoCanAct)
    for (let [i, t] of this.turns.entries()) {
      const flagRoleRaw = t?.flags?.d100A?.crewRole;
      const flagRole = flagRoleRaw != null ? String(flagRoleRaw).trim() : "";
      const crewRole = (flagRole ? flagRole : null) ?? t?.crewRole;
      const crewRoleNormalized = crewRole != null ? String(crewRole).toLowerCase() : null;
      ...
      else if (crewRoleNormalized && whoCanActNormalized.includes(crewRoleNormalized)) {
  ...
  async GMUpdate() {
    ...
    if (true) {
      ...
      do {
        ...
        // Ordnance migration: decrement duration-lifetime on Piloting subphase.
        await this._tickOrdnanceLifetimePiloting();
  ...
  async delayTurn(combatantId) {
  await this.nextTurn(true)
  }
  ...
  async nextTurn(skip = false) {
    ...
    const updateOptions = {};
    const piloting = ["vehicle", "starship"].includes(this.flags.d100A.combatType) && (this.flags.d100A.action == 0);


    if (!skip && !!oldcombatant && !piloting) oldcombatant.actionsRemaining = oldcombatant.actionsRemaining - 1;
    ...
    try {
      if (!game.user.isGM) {
        ...
        const notice = game.i18n?.localize?.("Alternityd100.Combat.LastTurnInPhase") || "No more valid turns in this phase. Please advance.";
      ...
      console.error("d100Combat: failed to notify GM of last turn in phase", err);
    }
 ...
 //   console.log(this)
  ...
  async _printNewRoundChatCard(eventData) {
    ...
    const speakerName = game.i18n.format(d100Combat.chatCardsText.speaker.GM);
    const templateData = {
      header: {
        image: actionImage,
        name: game.i18n.format(d100Combat.chatCardsText.round.headerName, { round: this.round })
      },
      body: {
        header: game.i18n.format(d100Combat.chatCardsText.round.bodyHeader),
        headerColor: d100Combat.colors.round
      },
      footer: {
        content: game.i18n.format(d100Combat.chatCardsText.footer, { combatType: localizedCombatName, combatPhase: localizedPhaseName })
      }
    };
  ...
  async _printNewPhaseChatCard(eventData) {
    ...
    const templateData = {
      header: {
        image: actionImage,
        name: game.i18n.format(d100Combat.chatCardsText.phase.headerName, { phase: localizedPhaseName })
      },
      body: {
        header: localizedPhaseName,
        headerColor: d100Combat.colors.phase,
        message: {
          title: game.i18n.format(d100Combat.chatCardsText.phase.messageTitle),
  ...
  async _printNewTurnChatCard(eventData) {
    ...
    let characterName = game.i18n.format(d100Combat.chatCardsText.turn.headerName, { combatant: eventData.newCombatant.name })
    if (eventData.combat.flags.d100A.combatType == "starship") {
      characterName = game.i18n.format(d100Combat.chatCardsText.turn.headerName, { combatant: eventData.newCombatant.actor.name })
    ...
    const templateData = {
      ...
      body: {
        header: "",
        headerColor: d100Combat.colors.turn,
        message: {
  ...
  getCombatName() {
    switch (this.getCombatType()) {
      default:
      case "normal":
        return game.i18n.format(d100Combat.normalCombat.name);
      case "starship":
        return game.i18n.format(d100Combat.starshipCombat.name);
      case "vehicleChase":
        return game.i18n.format(d100Combat.vehicleChase.name);
    }
  }
  ...
  getPhases() {
    switch (this.getCombatType()) {
      default:
      case "normal":
        return d100Combat.normalCombat.phases;
      case "starship":
        return d100Combat.starshipCombat.phases;
      case "vehicleChase":
        return d100Combat.vehicleChase.phases;
    }
  }
  ...
  getSubPhases() {
    switch (this.getCombatType()) {
      default:
      case "normal":
        return d100Combat.normalCombat.subPhases;
      case "starship":
        return d100Combat.starshipCombat.subPhases;
      case "vehicleChase":
        return d100Combat.vehicleChase.subPhases;
    }
  }
    ...
    do {
      ...
      for (let [index, combatant] of this.turns.entries()) {
        ...
        if (thisSubPhase.piloting) {  // For Pilots in pilot Phase
          console.log("\nPilot - ",nextSubPhase.whoCanAct, combatant.flags.d100A.crewRole, combatant.actor.name, thisSubPhase.whoCanAct)
          if ((!combatant.defeated) && thisSubPhase.whoCanAct.includes(combatant.flags.d100A.crewRole)) {
            //console.log("\nPilot - ",combatant.actor.name)
            return nextTurn;
          }
        }
        if (!thisSubPhase.piloting) {// for normal actions    
          if ((!combatant.defeated) && combatant.flags.d100A?.canAct) {
            console.log("\n-----ValidTurn-----:",index)
            return nextTurn;
          }
        }
...
if (combatant.flags.d100A.actions.actedThisPhase) {
 ...
 /*     if (result && combatant.flags.dragRuler) {
  ...
  _sortCombatantsAsc(a, b) {
    ...
    xxrenderCombatTypeControls(html) {
 ...
 combattypeprev() {
    console.log("combat-type-prev clicked");
    onConfigClicked(this, -1);
    this.render();
}
combattypenext() {
    console.log("combat-type-next clicked");
    onConfigClicked(this, 1);
    this.render();
}
}
...
Hooks.on('xrenderCombatTracker', (app, html, data) => {
  //const html = htmlx[0]; //jquery
  ...
  const roundHeader = header.querySelector('h3');
//  console.log(header);
  const originalHtml = roundHeader;

  if (activeCombat.round) {
    const phases = activeCombat.getPhases();
    if (phases.length > 1) {
      const phasename = document.createElement("div");
      phasename.classList.add('phase-title', 'combat-tracker-header', 'Alternityd100');
      header.appendChild(phasename);
      phasename.append(`${game.i18n.format(activeCombat.getCurrentPhase().name)} - ${game.i18n.format(activeCombat.getCurrentSubPhase().name)}`);
...
d100Combat.starshipCombat = {
  ...
  subPhases: [

    {
      ...
      iterateTurns: true,
      resetInitiative: false,
      whoCanAct: ["Pilot", "Copilot", "pilot", "copilot", "ordnance"],
      piloting: true,
      killActors: false
    },
    {
