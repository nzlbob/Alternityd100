...
*/
        ...
        */
        ...
        for (const component of starshipComponents) {
            const componentData = component.system;
            let armorbhpCost = 0;
            const isstarshipArmor = componentData.type === "starshipArmor";
            if (isstarshipArmor) {

                armorbhpCost = Math.max(parseInt(componentData.price) + Math.round(parseInt(componentData.pricePerBHP) * parseInt(data.attributes.hullPoints.base) * parseInt(componentData.bhpCost) / 100), 1);
            }
           ...
           // console.log("------PRICE COST--------\n",component.name, componentData.quantity, isNaN(parseInt(componentData.quantity)),parseInt(componentData.quantity))
            const quantity =  isNaN(parseInt(componentData.quantity)) ? 1 : Math.max( parseInt(componentData.quantity), 1);
           // console.log("------PRICE COST--------\n",component.name, quantity)
            let Price = isstarshipArmor ? armorbhpCost : (parseInt(componentData.price) + parseInt(componentData.pricePerBHP) * parseInt(componentData.bhpCost)) * quantity;
