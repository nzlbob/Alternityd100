...
/*
        ...
        /** Set up base values. */
/*        const forwardTL = foundry.utils.duplicate(data.quadrants.forward.targetLock);
        ...
        const portTL = foundry.utils.duplicate(data.quadrants.port.targetLock);
        ...
        const starboardTL = foundry.utils.duplicate(data.quadrants.starboard.targetLock);
        ...
        const aftTL = foundry.utils.duplicate(data.quadrants.aft.targetLock);
        ...
        /** Get modifying items. */
 /*       let defenceItem = null;
        const defenceItems = fact.items.filter(x => x.type === "starshipDefence");
        if (defenceItems && defenceItems.length > 0) {
            defenceItem = defenceItems[0];
        }

        let electronicCountermeasureItem = null;
        const electronicCountermeasureItems = fact.items.filter(x => x.type === "starshipElectronicCountermeasure");
        if (electronicCountermeasureItems && electronicCountermeasureItems.length > 0) {
            electronicCountermeasureItem = electronicCountermeasureItems[0];
        }
...
/*        const addScore = (target, title, value, bLocalize = true) => {
        ...
        if (defenceItem) {
            const DefenceData = defenceItem.system;
            addScore(data.quadrants.forward.targetLock, defenceItem.name, DefenceData.targetLockBonus, false);
            addScore(data.quadrants.port.targetLock, defenceItem.name, DefenceData.targetLockBonus, false);
            addScore(data.quadrants.starboard.targetLock, defenceItem.name, DefenceData.targetLockBonus, false);
            addScore(data.quadrants.aft.targetLock, defenceItem.name, DefenceData.targetLockBonus, false);
        }

        if (electronicCountermeasureItem) {
            const electronicCountermeasureData = electronicCountermeasureItem.system;
